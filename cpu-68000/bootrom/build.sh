...
vasmm68k_mot -nowarn=2028 -Fbin -m68000 -esc -spaces -o ebasic/ebasic.bin -L ebasic/ebasic.lst ebasic/basic.68k
...
vasmm68k_mot -Fbin -m68000 -esc -spaces -o apps/directory.bin -L apps/directory.lst apps/directory.68k

# build VUBUG
echo "\n\n${bold}Assembling VUBUG${normal}"
vasmm68k_mot -Fbin -m68000 -esc -spaces -o vubug/vubug.bin -L vubug/vubug.lst vubug/VUBUG.68k
dd if=vubug/vubug.bin of=vubug/vubug_trimmed.bin skip=14680064 bs=1 conv=notrunc > /dev/null 2>&1
...
dd if=vubug/vubug_trimmed.bin of=rom_vubug.bin seek=0 bs=1 count=32768 conv=notrunc > /dev/null 2>&1
