...
*************************************************************************************
    ...
    ; the app is loaded to RAM at $F00000
    org             $F00000
    section code,code
...
VEC_IN
    ; make SvcTtyGetch call
    moveq       #6, d0
    trap        #15

    ; if d0 is negative, no character
    tst.w       d0
    bmi.s       .none

.read:
    ; set carry, character available (it's in d0)
    ori.b       #1, ccr
    rts

.none:
    ; clear carry, no character available
    andi.b      #$fe, ccr
    rts
...
LAB_1359
	...
	BEQ.s		LAB_1378		 ; branch if [BELL]
        ...
        ; Ctrl+D to exit back to monitor
        cmp.b   #$4, d0
        beq.s ExitToMonitor

	CMP.b		#$0D,d0		 ; compare with [CR]
	BEQ		LAB_1866		 ; do CR/LF exit if [CR]
	CMP.b		#$0A,d0		 ; compare with [LF]
	BEQ		LAB_1866		 ; do CR/LF exit if [LF]
...
LAB_137F
    ; this line controls whether characters are echoed
	;BSR		LAB_PRNA		 ; go print the character
